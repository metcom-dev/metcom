...
class PleAssetsBook(models.Model):
    ...
    def _get_lines_1(self):
        ...
        COALESCE(calculate_line_state_name(aa.id),'') as cuo_c_state
            ...
            for var_list in list_data:
                if asset['asset_id'] == var_list['id']:
                    ...
                    if asset['state'] == 'close':
                        ...
                        function_lines()
                        values_update_close_line_2 = ({'cuo': re.sub(r"[^a-zA-Z0-9]", "", asset['cuo_c_state']),
                                                       'correlative': re.sub(r"[^a-zA-Z0-9]", "",
                                                                             asset['number_correlative_c_state']),
                                                       'asset_opening': 0.00,
                                                       'accumulated_depreciation_end_previous_year': 0.00,
                                                       'asset_amount': 0.00, })
                        lines[-1].update(values_update_close_line_2)
            if asset['parent_id']:
                function_lines()
