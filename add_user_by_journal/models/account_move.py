        ...
        if view_type == 'form':
            ...
            for node in doc.xpath(value):
                if user.has_group('add_user_by_journal.group_assign_to'):
                    node.attrib['domain'] = f"['&', ('id', 'in', suitable_journal_ids), ('type', 'in', ('purchase','general','situation','sale','bank','cash'))]"
                else:
                    node.attrib[
                        'domain'] = f"['&', '&', ('id', 'in', suitable_journal_ids),('assign_to', 'in', {user.id}) , ('type', 'in', ('purchase','general','situation','sale','bank','cash'))]"
