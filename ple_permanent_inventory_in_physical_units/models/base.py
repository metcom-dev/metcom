...
class AccountAccount(models.Model):
    ...
    ple_selection = fields.Selection(
        selection_add=[('stock_revaluation_book', '12.1 y 13.1 Registro del Inventario permanente')])
...
class StockLocation(models.Model):
    ...
    correlative = fields.Integer(string='Correlativo')

    storehouse_id = fields.Many2one(comodel_name='stock.warehouse', string="Almacen")
...
class StockPickingType(models.Model):
    ...
    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(StockPickingType, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                            submenu=submenu)
        if view_type == 'form':
