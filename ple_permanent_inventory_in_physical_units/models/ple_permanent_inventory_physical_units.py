...
class PlePermanentInventoryPhysicalUnits(models.Model):
    ...
    def action_generate_excel(self):
        ...
        query = """
                ...
                replace(am.name,'/','') as cou,
                pp.id as product_id,
                svl.id as stock_valuation,
                (
                SELECT
                CASE aa.ple_selection
                WHEN 'stock_revaluation_book' THEN aml.ple_correlative
                ELSE 'M000000001' END as correlative
                FROM account_move am
                LEFT JOIN account_move_line aml ON am.id = aml.move_id
                LEFT JOIN account_account  aa ON aml.account_id = aa.id
                WHERE (am.id = svl.account_move_id) AND (LEFT(aa.code,2)='20')
                ) as correlative,
