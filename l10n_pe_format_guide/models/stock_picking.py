...
class StockPicking(models.Model):
    ...
    def _cal_weight(self):
        for picking in self:
            picking.weight = sum(move_line.reserved_uom_qty * move_line.product_id.weight for move_line in picking.move_line_ids if move_line.state != "cancel")
    ...
    def concatenated_fields_arrival(self):
        if self.partner_id.l10n_pe_district.name and self.partner_id.city and self.partner_id.state_id.name:
            return str(self.partner_id.l10n_pe_district.name) + "/" + str(self.partner_id.city) + "/" + str(self.partner_id.state_id.name)
        return ""
