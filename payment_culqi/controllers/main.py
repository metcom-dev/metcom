...
class CulqiController(http.Controller):
    ...
    def culqi_form_feedback(self, **data):
        ...
        amount_value = 'Pagar {}{} {}'.format(
            ...
            obj_transaction.currency_id.name or data.get('currency')
        )
    ...
    @http.route('/payment/culqi/return', type='http', auth="none", methods=['POST'], csrf=False, save_session=False)
    def culqi_return_from_checkout(self, **data):
        """ Process the notification data sent by Culqi after redirection from checkout.
        ...
        post_data = self.culqi_verification_data(data)
    ...
    @staticmethod
    def culqi_verification_data(data):
        expiry_list = data['expiry'].replace(' ', '').split("/")
